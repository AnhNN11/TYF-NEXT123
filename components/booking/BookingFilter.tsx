...
export default function DatePickerWithRange({
  ...
  const [selectedBabysitter, setSelectedBabysitter] =
    React.useState<Babysitter | null>(null);
  ...
  const handleInputChange = (
    ...
    if (name === "hours" || name === "months") {
      if (!isNaN(numberValue) && numberValue > 0) {
        ...
        const newTotalAmount = calculateAmount(
          updatedUserInfo.hours,
          updatedUserInfo.months,
          bookingType
        );
  ...
  const calculateAmount = (hours: number, months: number, type: string) => {
    if (type === "hourly") return hours * 49000;
    if (type === "monthly") return months * 4000000;
    return 0;
  };
  ...
  return (
    <div
      className={cn("grid gap-4 p-4 bg-white shadow-lg rounded-lg", className)}
    >
      {/* Select Type Booking */}
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {babysitters.map((babysitter) => (
          <Card
            key={babysitter.id}
            className="py-4 shadow-lg hover:shadow-xl transition-shadow"
          >
            <CardHeader className="pb-0 pt-2 px-4 flex-col items-center text-center">
              <CardTitle className="text-tiny uppercase font-light text-gray-600">
                Babysitter
              </CardTitle>
            ...
            <CardFooter className="flex justify-center">
              <Button
                variant="outline"
                className="w-full bg-green-500 text-white hover:bg-green-600 justify-center items-center"
                onClick={() => handleBookingClick(babysitter)}
              >
                Đặt lịch
              </Button>
            </CardFooter>
        ...
        <Dialog open={isModalOpen} onOpenChange={() => setIsModalOpen(false)}>
          <DialogContent className="max-w-xl">
            ...
            <p className="mt-4 text-lg">
              Tổng tiền:{" "}
              <strong>{userInfo.totalAmount.toLocaleString()} VND</strong>
            </p>
            <p className="mt-2 text-sm text-red-600">
              <strong>Lưu ý:</strong>{" "}
              {bookingType === "hourly"
                ? "Bạn cần thanh toán đầy đủ số tiền khi đặt theo giờ."
                : "Bạn chỉ cần thanh toán 50% số tiền khi đặt theo tháng."}
            </p>
